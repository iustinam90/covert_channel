...
# Covert channel using CPU/bandwidth. Tested with KVM 
...
def main():
    global bw_max_0,bw_min_1,ncat_srv,ncat_srv_port
    ...
    if(sys.argv[1]=='bench'):
        if(mode=='bw'):
            if(len(sys.argv)<5): print usage; sys.exit()
            ncat_srv=sys.argv[3]
            ncat_srv_port=int(sys.argv[4])
        bench(int(sys.argv[2])) 
        sys.exit()

    if(len(sys.argv)<4): print usage; sys.exit()
    bw_max_0=int(sys.argv[2])
    bw_min_1=int(sys.argv[3])
    if(bw_max_0<bw_min_1):
        diff1=(bw_min_1-bw_max_0)/2
        bw_max_0+=diff1
        bw_min_1-=diff1
        print 'changed bw_max_0',bw_max_0,'bw_min_1',bw_min_1,'to fill the gap'
    if(mode=='bw'):
        if(len(sys.argv)<6): print usage; sys.exit()
        ncat_srv=sys.argv[4]
        ncat_srv_port=int(sys.argv[5])
